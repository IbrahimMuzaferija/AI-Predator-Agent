...
public class GeneticAlgorithm<I extends Individual, P extends Problem<I>> {
    ...
    public I run(P problem) {
        ...
        while (!stopCondition(t)){
            ...
            // select best
            Population<I,P> selectedPopulation = selection.run(population);

            // evolve them
            recombination(selectedPopulation);
            mutation(selectedPopulation);

            // integrate with the rest of population
            population = generateNewPopulation(population, selectedPopulation);

            // select new best individual
            I bestInGen = population.evaluate();

            // compare them
            if (bestInGen.getFitness() > bestInRun.getFitness()){
                bestInRun = (I) getBestInGeneration().clone();
            }
    ...
    public Population generateNewPopulation(Population<I, P> current, Population<I, P> next) {
        next.replaceWorst(current.getBest()); //Elitism
        return next;
    }
